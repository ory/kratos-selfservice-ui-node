...
async function scanMeasures(directoryName: string) {
    ...
    for (let f of files) {
        let fullPath: string = path.join(directoryName, f.name);
        if(f.name.endsWith(".xml"))
        {
            ...
            m.date = date;
            m.fileName = fullPath;
            if((date - startingDate) > 0) // starting Apr 17 2021 is when we have both file available in sync
                measures.push(m);
        }
    }
