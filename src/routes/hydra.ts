...
const createHydraSession = (
  ...
  const verifiable_addresses = context?.identity?.verifiable_addresses || []
  if (
    requestedScope.indexOf('email') === -1 ||
    verifiable_addresses.length === 0
  ) {
    return {}
  }
  ...
  return {
    ...
    idToken: {
      email: verifiable_addresses[0].value as Object, // FIXME Small typescript workaround caused by a bug in Go-swagger
    },
  }
}
    ...
    .then(({ body }) => {
      ...
      if (body.skip) {
        ...
        acceptConsentRequest.session = createHydraSession(
          body.requestedScope,
          body.context as Session | undefined
        )
